...
function isValidTelegramIP(request) {
  const trustedTelegramIPs = [
    "149.154.160.0/20",
    "91.108.4.0/22"
  ];
  const ip = request.headers.get("cf-connecting-ip");
  if (!ip) return false;
  return trustedTelegramIPs.some(cidr => isIPInCIDR(ip, cidr));
}
  ...
  async fetch(request, env) {
    ...
    async function autoRegisterWebhook(request) {
      ...
      await fetchWithRetry(`https://api.telegram.org/bot${BOT_TOKEN}/setWebhook`, {
        ...
        body: JSON.stringify({ url: webhookUrl, secret_token: env.TELEGRAM_SECRET_TOKEN }),
      });
    }
